...
namespace ht
{
	...
	void CALLBACK snapAsync(LPVOID prm, DWORD, DWORD)
	{
		...
		if (snap(htHandle))
			std::cout << "spanshotAsync success (CALLBACK func)" << std::endl;
	}

	HtHandle* open(const wchar_t* fileName)         // имя файла
	{
		HtHandle* htHandle = openHtFromFile(fileName); //открытие
		if (htHandle == NULL)
		{
			htHandle = openHtFromMapName(fileName); //создание
			if (htHandle == NULL)
				return NULL;
		}
	...
	HtHandle* openHtFromMapName(const wchar_t* fileName)
	{
		...
		if (!UnmapViewOfFile(lp))	return NULL;
		if (!CloseHandle(hm))	return NULL;
	...
	//  читать элемент из хранилища
	Element* get(HtHandle* htHandle, const Element* element)          // управление HT     // элемент 
	{
		int index = findIndex(htHandle, element);
		if (index < 0)
		{
			writeLastError(htHandle, "ERROR: not found element (GET)");
			return NULL;
		}
	...
	BOOL insert(HtHandle* htHandle, const Element* element)           // управление HT   // элемент
	{
		if (htHandle->count >= htHandle->capacity)
		{
			writeLastError(htHandle, "ERROR: not found free memory");
			return false;
		}
	...
	BOOL update(HtHandle* htHandle, const Element* oldElement, const void* newPayload, int newPayloadLength)     // размер новых данных
	{
		WaitForSingleObject(htHandle->mutex, INFINITE);
		int index = findIndex(htHandle, oldElement);
		if (index < 0)
		{
			writeLastError(htHandle, "ERROR:  not found element (UPDATE)");
			ReleaseMutex(htHandle->mutex);
			return false;
		}
	...
	BOOL remove(HtHandle* htHandle, const Element* element)
	{
		WaitForSingleObject(htHandle->mutex, INFINITE);
		int index = findIndex(htHandle, element);
		if (index < 0)
		{
			writeLastError(htHandle, "ERROR:  not found element (DELETE)");
			ReleaseMutex(htHandle->mutex);
			return false;
		}
	...
	Element* readFromMemory(const HtHandle* const htHandle, Element* const element, int index)
	{
		...
		lp = (HtHandle*)lp + 1;
		lp = (byte*)lp + getSizeElement(htHandle->maxKeyLength, htHandle->maxPayloadLength) * index; //сдвигаем указатель lp на нужный элемент данных, который хранится в памяти

		element->key = lp;
		lp = (byte*)lp + htHandle->maxKeyLength;													//Указатель lp сдвигается на следующий элемент данных, который представляет длину ключа элемента
		element->keyLength = *(int*)lp;
		lp = (int*)lp + 1;																			//Указатель lp сдвигается на следующий элемент данных(payload)
		element->payload = lp;
		lp = (byte*)lp + htHandle->maxPayloadLength;												//казатель lp сдвигается на следующий элемент данных(payloadLength)
	...
	const char* getLastError(const HtHandle* htHandle)				// управление HT
	{
		return htHandle->lastErrorMessage;
	}
}