...
#define PIPE_NAME_LAN L"\\\\.\\pipe\\Tube"
...
int main()
{
	...
	try
	{
		...
		// и разрыва связи 
		if (!CallNamedPipe(
			PIPE_NAME, // канал // [in] дескриптор канала
			buffer, //входной буфер // [in] указатель на буфер для записи
			sizeof(buffer), // размер входного буфера // [in] размер буфера для записи
			outbuffer, //[out] указатель на буфер для чтения
			MAX_SIZE_OF_BUFFER, // размер выходного буфера // [in] размер буфера для чтения
			&bytes, // [out] количество прочитанных байт
