...
int main()
{
	...
	try
	{
		// 1 блок - подсоединение клиента

		if ((cH = CreateFile(
			MAILSLOT_NAME,																																																		// [in] символическое имя почтового ящика
			GENERIC_WRITE,																																																		// [in] чтение или запись
			FILE_SHARE_READ | FILE_SHARE_WRITE,																																													// [in] режим совместного использования
			NULL,																																																				// [in] атрибуты безопасности
			OPEN_EXISTING,																																																		// [in] флаг открытия почтового ящика
			NULL,																																																				// [in] флаги и атрибуты
			NULL)) == INVALID_HANDLE_VALUE)																																														// [in] дополнительные атрибуты
		{
		...
		char writeBuf[500] = "Hello from Maislot-client"; // для блока 2
		DWORD writeMsg;
